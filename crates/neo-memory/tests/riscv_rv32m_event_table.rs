...
#[test]
fn rv32m_event_table_is_empty_when_program_has_no_rv32m_ops() {
    // Program: ADDI; HALT
    let program = vec![
        RiscvInstruction::IAlu {
            op: RiscvOpcode::Add,
    ...
    assert!(events.rows.is_empty(), "expected no RV32M events");
}

#[test]
fn rv32m_event_table_single_row_for_single_mul() {
    ...
    assert_eq!(events.rows.len(), 1, "expected exactly one RV32M event");
    let e = &events.rows[0];
    assert_eq!(e.opcode, RiscvOpcode::Mul);
    assert_eq!(e.rs1, 0);
    assert_eq!(e.rs2, 0);
    assert_eq!(e.rd, 1);
    assert_eq!(e.rs1_val, 0);
    assert_eq!(e.rs2_val, 0);
    assert_eq!(e.expected_rd_val, 0);
    assert_eq!(e.rd_write_val, Some(0));
}
