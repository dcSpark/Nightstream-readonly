...
fn semantic_constraints(layout: &Rv32B1Layout, mem_layouts: &HashMap<u32, PlainMemLayout>) -> Result<Vec<Constraint<F>>, String> {
    ...
    for j in 0..layout.chunk_size {
        ...
        // Range-check division outputs to keep quotient/remainder canonical.
        enforce_u32_bits(
            &mut constraints,
            one,
            layout.div_quot(j),
            layout.div_quot_bits_start,
            layout.chunk_size,
            j,
        );
        enforce_u32_bits(
...
pub fn build_rv32_b1_step_ccs(
    ...
    let layout = loop {
        match build_layout_with_m(m, mem_layouts, &table_ids, chunk_size) {
            Ok(layout) => break layout,
            Err(e) if e.contains("need more padding columns before bus tail") || e.contains("overlaps public inputs") => {
                m = m.checked_mul(2).ok_or_else(|| "RV32 B1: m overflow".to_string())?;
            }
            Err(e) => return Err(e),
        }
    };
