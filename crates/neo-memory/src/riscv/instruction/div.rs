...
impl InstructionDescriptor for Div {
    fn opcode() -> Option<RiscvOpcode> {
        Some(RiscvOpcode::Div)
    }
...
pub fn eval(rs1: u64, rs2: u64, xlen: usize) -> u64 {
    compute_op(RiscvOpcode::Div, rs1, rs2, xlen)
}

/// Jolt-style signed DIV decomposition (with one virtual self-move row for commit linkage).
