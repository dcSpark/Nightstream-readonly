...
impl InstructionDescriptor for Remu {
    fn opcode() -> Option<RiscvOpcode> {
        Some(RiscvOpcode::Remu)
    }
...
pub fn eval(rs1: u64, rs2: u64, xlen: usize) -> u64 {
    compute_op(RiscvOpcode::Remu, rs1, rs2, xlen)
}

/// Jolt-style REMU decomposition sequence.
///
/// ```text
/// ADVICE               -> v_q
...
pub fn decomposition_sequence(rd: u8, rs1: u8, rs2: u8, alloc: &mut VirtualRegisterAllocator) -> Vec<DecomposedOp> {
    ...
    vec![
        ...
        DecomposedOp::Move { dst: v_rem, src: v_rem },
        DecomposedOp::Move { dst: rd, src: v_rem },
    ]
}
