...
fn test_direct_sum_polynomial_mixing_concern() {
    ...
    let terms1 = vec![Term { coeff: F::ONE, exps: vec![1] }]; // f1(y) = y[0]
    ...
    let terms2 = vec![Term { coeff: -F::ONE, exps: vec![1] }]; // f2(y) = -y[0]
...
fn test_direct_sum_mixed_prevents_cancellation() {
    ...
    let terms1 = vec![Term { coeff: F::ONE, exps: vec![1] }]; 
    let f1 = SparsePoly::new(1, terms1);
    let m1 = Mat::from_row_major(1, 2, vec![F::from_u64(5), F::ZERO]);
    let ccs1 = CcsStructure::new(vec![m1], f1).unwrap();
    
    let terms2 = vec![Term { coeff: -F::ONE, exps: vec![1] }];
    let f2 = SparsePoly::new(1, terms2);  
    ...
    for beta_val in [2u64, 3u64, 42u64, 123u64] {
        ...
        assert!(result.is_err(), "Mixed sum should prevent cancellation for β = {}", beta_val);
        
        println!("✅ SECURITY: Mixed direct sum with β={} prevents cancellation", beta_val);
        println!("   f_total = 5*(1-{}) = {} ≠ 0", beta_val, 5_u64.wrapping_mul(1_u64.wrapping_sub(beta_val)));
    }
}
...
fn test_transcript_bound_mixing() {
    ...
    let terms2 = vec![Term { coeff: F::from_u64(2), exps: vec![1] }];
    let f2 = SparsePoly::new(1, terms2);  
...
fn test_mixed_direct_sum_constraint_preservation() {
    ...
    let f1 = SparsePoly::new(1, vec![Term { coeff: F::ONE, exps: vec![1] }]);
    let m1 = Mat::from_row_major(2, 3, vec![
        F::ONE, F::ZERO, F::from_u64(2),  // row 0: z[0] + 2*z[2] 
        F::ZERO, F::ONE, F::from_u64(3),  // row 1: z[1] + 3*z[2]
    ]);
    let ccs1 = CcsStructure::new(vec![m1], f1).unwrap();
    
    // CCS2: 1 constraint, 2 variables
    let f2 = SparsePoly::new(1, vec![Term { coeff: F::from_u64(4), exps: vec![1] }]);
    let m2 = Mat::from_row_major(1, 2, vec![
        F::from_u64(5), F::from_u64(6),  // row 0: 5*z[0] + 6*z[1]
    ]);
    let ccs2 = CcsStructure::new(vec![m2], f2).unwrap();
