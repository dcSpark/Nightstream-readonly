...
pub fn r1cs_to_ccs<F: Field>(
    a: Mat<F>, b: Mat<F>, c: Mat<F>
) -> CcsStructure<F> {
    assert_eq!(a.rows(), b.rows()); assert_eq!(a.rows(), c.rows());
    assert_eq!(a.cols(), b.cols()); assert_eq!(a.cols(), c.cols());

    // f(X1,X2,X3) = X1^1 * X2^1 + (-1)*X3^1
    let t = 3usize;
    let terms = vec![
