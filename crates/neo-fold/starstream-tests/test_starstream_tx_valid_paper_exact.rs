...
//! Integration test for Starstream TX (paper-exact fixture; optimized folding mode)
...
fn verify_identity_first(s: &CcsStructure<F>) {
    ...
    assert!(m1.rows() == s.n, "M₁ must have n={} rows, but has {}", s.n, m1.rows());
    assert!(
        m1.cols() >= s.n,
    ...
    match m1 {
        CcsMatrix::Identity { n } => {
            assert_eq!(*n, s.n, "M₁ identity dimension mismatch: expected n={}, got {}", s.n, n);
            assert_eq!(
                m1.cols(),
                s.n,
        ...
        CcsMatrix::Csc(csc) => {
            ...
            for c in 0..m1.cols() {
                ...
                if c < s.n {
                    ...
                    assert_eq!(csc.vals[k], F::ONE, "M₁ identity entry at ({},{}) must be 1", c, c);
                } else {
                    assert_eq!(e0, s0, "M₁ column {} (beyond n={}) should be all-zero", c, s.n);
                }
            }
        }
    }
...
fn test_starstream_tx_valid_paper_exact() {
    ...
    let ok = session
        .verify(&baseline_ccs, &mcss_public, &run)
