...
fn shared_bus_buslen_and_constraints(
    ...
    shout_cpu: &HashMap<u32, ShoutCpuBinding>,
    twist_cpu: &HashMap<u32, TwistCpuBinding>,
) -> Result<(usize, usize), String> {
    ...
    // Compute ell_addr (d * ell) for each instance.
    ...
    let bus_cols = shout_ell_addrs
        .iter()
        .map(|&ell| ell + 2)
        .chain(twist_ell_addrs.iter().map(|&ell| 2 * ell + 5))
        .sum::<usize>();
    ...
    let bus_layout = neo_memory::cpu::build_bus_layout_for_instances(
        ...
        m_in,
        chunk_size,
        shout_ell_addrs.iter().copied(),
        twist_ell_addrs.iter().copied(),
    )?;
    ...
    for (i, table_id) in table_ids.iter().enumerate() {
        ...
        builder.add_shout_instance_bound(&bus_layout, &bus_layout.shout_cols[i], cpu);
    }
    for (i, mem_id) in mem_ids.iter().enumerate() {
        let cpu = twist_cpu
            .get(mem_id)
            .ok_or_else(|| format!("missing twist_cpu binding for mem_id={mem_id}"))?;
        builder.add_twist_instance_bound(&bus_layout, &bus_layout.twist_cols[i], cpu);
    }
