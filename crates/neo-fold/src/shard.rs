    ...
    for (idx, (step, step_proof)) in steps.iter().zip(proof.steps.iter()).enumerate() {
            ...
            if shared_cpu_bus {
                ...
                for (mem_idx, proof) in step_proof.val_fold.iter().enumerate() {
                    ...
                    if has_prev {
                        ...
                        .map_err(|e| {
                            PiCcsError::ProtocolError(format!(
                                "step {} val_fold(no-shared, mem_idx={}, with_prev) verify failed: {e:?}",
                                idx, mem_idx
                            ))
                        })?;
                    } else {
                        ...
                        .map_err(|e| {
                            PiCcsError::ProtocolError(format!(
                                "step {} val_fold(no-shared, mem_idx={}, cur_only) verify failed: {e:?}",
                                idx, mem_idx
                            ))
                        })?;
                    }
        ...
        if step_proof.mem.twist_me_claims_time.is_empty() {
                ...
                .enumerate()
            {
                ...
                .map_err(|e| {
                    PiCcsError::ProtocolError(format!(
                        "step {} twist_time_fold mem_idx {} verify failed: {e:?}",
                        idx, mem_idx
                    ))
                })?;
                val_lane_obligations.extend_from_slice(&proof.dec_children);
