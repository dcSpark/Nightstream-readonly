...
fn test_session_single_fold_with_crosscheck() {
    ...
    let crosscheck_cfg = CrosscheckCfg {
        ...
        terminal: true,     // Re-enable to see mismatch details
        outputs: true,      // Check output instances
    };

    // Session with Accumulator (k=2: 1 accumulated ME + 1 new MCS) using crosscheck mode
    ...
    println!("Prove step succeeded, now calling finalize...");
    let run = match session.finalize(&step.ccs) {
        Ok(r) => {
            println!("Finalize succeeded!");
            r
        },
        Err(e) => {
            eprintln!("Finalize failed: {:?}", e);
            panic!("finalize should produce a FoldRun: {:?}", e);
        }
    };

    println!("Finalize completed successfully!");
    println!("Run has {} steps", run.steps.len());
    println!("Run has {} final outputs", run.final_outputs.len());

    // NOTE: Verification always uses the optimized verifier (crosscheck is prover-side only)
    // So we expect verification to work normally.
