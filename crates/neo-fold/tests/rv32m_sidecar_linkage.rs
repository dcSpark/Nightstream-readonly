...
use neo_memory::riscv::ccs::build_rv32_b1_rv32m_event_sidecar_ccs;
...
fn rv32m_sidecar_is_bound_to_main_witness_commitment() {
    ...
    // Build the RV32M event sidecar CCS for lane 0 of a chunk_size=1 execution.
    let rv32m_ccs = build_rv32_b1_rv32m_event_sidecar_ccs(run.layout(), &[0usize]).expect("build rv32m sidecar ccs");

    // Prove/verify only the first chunk (the RV32M instruction).
    let step0 = &run.steps_witness()[0];
    let (inst0, wit0) = &step0.mcs;
    let mcs_insts = vec![inst0.clone()];
    let mut mcs_wits = vec![wit0.clone()];
