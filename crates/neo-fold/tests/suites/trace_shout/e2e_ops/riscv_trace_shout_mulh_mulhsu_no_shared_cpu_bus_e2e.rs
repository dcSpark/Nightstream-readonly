...
fn build_shout_only_bus_z_packed_mulh(
    ...
    for j in 0..t {
        ...
        // [lhs_u32, rhs_u32, hi_u32, lhs_sign, rhs_sign, k∈{0,1,2}, lo_bits[0..32]].
...
fn build_shout_only_bus_z_packed_mulhsu(
    ...
    for j in 0..t {
        ...
        // Packed-key layout (ell_addr=37):
        // [lhs_u32, rhs_u32, hi_u32, lhs_sign, borrow∈{0,1}, lo_bits[0..32]].
...
fn riscv_trace_wiring_ccs_no_shared_cpu_bus_shout_mulh_mulhsu_packed_prove_verify() {
    ...
    // RV32 B1 does not currently emit MULH/MULHSU Shout events. Inject one per matching instruction row.
    ...
    let mut injected_mulh = 0usize;
    let mut injected_mulhsu = 0usize;
    ...
    assert!(injected_mulh > 0, "expected to inject at least one MULH Shout event");
    assert!(
        injected_mulhsu > 0,
        "expected to inject at least one MULHSU Shout event"
    );
    ...
    // Shout instances: MULH and MULHSU packed, 1 lane each.
    let t = exec.rows.len();
    ...
    let mut tr_prove = Poseidon2Transcript::new(b"riscv-trace-no-shared-bus-shout-mulh-mulhsu-packed");
    ...
    let mut tr_verify = Poseidon2Transcript::new(b"riscv-trace-no-shared-bus-shout-mulh-mulhsu-packed");
