...
fn riscv_trace_wiring_ccs_no_shared_cpu_bus_shout_mul_prove_verify() {
    ...
    // RV32 B1 does not currently emit MUL Shout events. Inject one per MUL instruction row so we can
    ...
    assert!(injected > 0, "expected to inject at least one MUL Shout event");
    ...
    // Shout instance: MUL table, 1 lane.
    let t = exec.rows.len();
    ...
    let mul_z = build_shout_only_bus_z(
        ccs.m,
        layout.m_in,
        t,
    ...
    let mut tr_prove = Poseidon2Transcript::new(b"riscv-trace-no-shared-bus-shout-mul");
    ...
    let mut tr_verify = Poseidon2Transcript::new(b"riscv-trace-no-shared-bus-shout-mul");
