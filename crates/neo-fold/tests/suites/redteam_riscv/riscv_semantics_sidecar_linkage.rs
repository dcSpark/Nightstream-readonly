...
fn tamper_semantics_related_scalar(proof: &mut ShardProof) {
    ...
    panic!("expected at least one semantics-related scalar in val claims");
}

#[test]
fn rv32_trace_semantics_tampered_scalar_must_not_verify() {
    let run = prove_run_addi_halt(/*imm=*/ 1);
    let mut bad_proof = run.proof().clone();
    tamper_semantics_related_scalar(&mut bad_proof);
    assert!(
        run.verify_proof(&bad_proof).is_err(),
        "semantics-related tamper must not verify"
    );
}

#[test]
fn rv32_trace_semantics_splicing_across_runs_must_fail() {
    let run_a = prove_run_addi_halt(/*imm=*/ 1);
    let run_b = prove_run_addi_halt(/*imm=*/ 2);
    assert!(
        run_a.verify_proof(run_b.proof()).is_err(),
        "spliced semantics commitments must not verify"
    );
}
