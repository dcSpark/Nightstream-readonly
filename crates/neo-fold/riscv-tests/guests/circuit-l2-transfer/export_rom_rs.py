...
def _format_u8_array(name: str, base: int, content: bytes, sha256_hex: str) -> str:
    lines = []
    lines.append("// @generated by crates/neo-fold/riscv-tests/guests/circuit-l2-transfer/export_rom_rs.py")
...
def main() -> int:
    ...
    out_rs = repo_root / "crates/neo-fold/riscv-tests/binaries/circuit_l2_transfer_rom.rs"

    subprocess.run(["cargo", "build", "--release"], cwd=guest_dir, check=True)

    elf = guest_dir / "target/riscv32im-unknown-none-elf/release/circuit_l2_transfer"
    if not elf.exists():
