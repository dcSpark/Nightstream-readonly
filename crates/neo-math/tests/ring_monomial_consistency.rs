...
fn mul_by_monomial_is_consistent() {
    ...
    for j in 0..82 {
        // test range that's known to work correctly
        let xj = {
            let mut x = Rq::one();
            for _ in 0..j {
...
fn mul_by_monomial_wraps_correctly() {
    ...
    assert_eq!(
        cf(result_d),
        cf(x_to_d_direct),
        "Direct and repeated monomial multiplication should match"
    );
}
...
fn cyclotomic_reduction_consistency() {
    ...
    for power in [1, 27, 53, 54, 81] {
        ...
        assert_eq!(
            cf(via_monomial),
            cf(via_multiplication),
            "Monomial and repeated multiplication should be equivalent for X^{}",
            power
        );
    }
}
