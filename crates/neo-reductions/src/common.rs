            ...
            if v != 0 {
                return Err(PiCcsError::ProtocolError(format!(
                    "DEC split: Z[{},{}] needs more than k_rho={} digits in base b={}\n\
                     ...
                     This typically means witness values grew too large during RLC (expansion factor T=216 for rotation matrices)",
                    r, c, k, b, Z_rows, Z_cols, k, v, -(B_u as i128), B_u as i128, b, k, B_u
                )));
            }
        }
    }
...
fn rot_from_coeffs(a_coeffs: &[F], phi_coeffs: &[i32]) -> Mat<F> {
    ...
    let neg_c: Vec<F> = phi_coeffs.iter()
...
pub fn sample_rot_rhos(
    ...
    if ring.phi_coeffs.len() != D {
        return Err(PiCcsError::InvalidInput(format!(
            "phi_coeffs length {} != D={}", ring.phi_coeffs.len(), D
        )));
    }
    ...
    if lhs >= b_pow_k {
        return Err(PiCcsError::InvalidInput(format!(
            ...
            k_rho+1, T, b-1, lhs, b_pow_k, params.k_rho, T
        )));
    }
    ...
    for i in 0..count {
        ...
        // Lift to field F
        let a_coeffs_f: Vec<F> = coeffs_i8.iter()
            .map(|&c| f_from_i64(c as i64))
            .collect();
