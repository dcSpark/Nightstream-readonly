...
impl<Ff: Field + PrimeCharacteristicRing + Copy + Send + Sync> SparseCache<Ff> {
    ...
    pub fn build(s: &CcsStructure<Ff>) -> Self {
        ...
        // Build per-matrix CSC. `from_dense_row_major` is already parallel over rows; keep this
        // loop sequential to avoid nested rayon parallelism.
        let mut csc: Vec<Option<CscMat<Ff>>> = Vec::with_capacity(t);
        for j in 0..t {
            if j == 0 {
                csc.push(None);
            } else {
                csc.push(Some(CscMat::from_dense_row_major(&s.matrices[j])));
            }
        }
