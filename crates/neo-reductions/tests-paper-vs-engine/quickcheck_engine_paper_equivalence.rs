...
fn ccs_structure_strategy() -> impl Strategy<Value = CcsStructure<F>> {
    (2usize..=8, 2usize..=8, 1usize..=3).prop_flat_map(|(n, m, t)| {
        // Generate t matrices
        ...
        // Generate polynomial
        let poly_strat = (1usize..=t)
            .prop_flat_map(move |num_terms| {
                prop::collection::vec(
...
fn neo_params_strategy() -> impl Strategy<Value = NeoParams> {
    (2u32..=4, 2u32..=4, 2u32..=4).prop_map(|(_, _, _)| {
...
fn test_full_pipeline_paper_vs_engine_k1() {
    ...
    let paper_sum = paper::sum_q_over_hypercube_paper_exact::<F>(
        &s, &params, &[wit.clone()], &[], &ch, ell_d, ell_n, None
    );
    ...
    let w_arr = [wit];
    let insts = precompute::prepare_instances(
        &s, &params, &[inst], &w_arr, &mats_csr, &l
    ).unwrap();
    let beta = precompute::precompute_beta_block(
        &s, &params, &insts, &w_arr, &[], &ch, ell_d, ell_n
    ).unwrap();
...
fn test_full_pipeline_paper_vs_engine_k2() {
    ...
    let f = SparsePoly::new(2, vec![
        ...
        Term { coeff: F::from_u64(2), exps: vec![0, 1] },
    ]);
    let s = CcsStructure::new(vec![m0, m1], f).unwrap();
    ...
    // Paper-exact
    let paper_sum = paper::sum_q_over_hypercube_paper_exact::<F>(
        &s, &params, &[wit0.clone()], &me_wits, &ch, ell_d, ell_n, None
    );
    ...
    let w_arr = [wit0];
    let insts = precompute::prepare_instances(
        &s, &params, &[inst0], &w_arr, &mats_csr, &l
    ).unwrap();
    let beta = precompute::precompute_beta_block(
        &s, &params, &insts, &w_arr, &me_wits, &ch, ell_d, ell_n
    ).unwrap();
...
fn test_rhs_terminal_comprehensive() {
    ...
    let f = SparsePoly::new(1, vec![
        Term { coeff: F::from_u64(3), exps: vec![2] },
        Term { coeff: F::from_u64(5), exps: vec![1] },
    ]);
    let s = CcsStructure::new(vec![m0], f).unwrap();
