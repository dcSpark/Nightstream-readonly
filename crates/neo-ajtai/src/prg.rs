...
//! Poseidon2(Goldilocks, w=12, r=8, cap=4) sponge. Host-only utility for now.
...
///
/// - Domain tag: b"neo/ajtai/prg/v1"
/// - Input bytes: domain || seed || row_idx_le || ctr_le
/// - Output: `len` field elements in Goldilocks (as F), produced in chunks of 4.
pub fn expand_row_v1(seed: &[u8; 32], row_idx: u64, len: usize) -> Vec<F> {
    let mut out = Vec::with_capacity(len);
    let mut ctr: u64 = 0;
    while out.len() < len {
        let mut bytes = Vec::with_capacity(b"neo/ajtai/prg/v1".len() + 32 + 8 + 8);
        bytes.extend_from_slice(b"neo/ajtai/prg/v1");
