...
fn masked_ct_matches_spec_various_patterns() {
    ...
    assert_eq!(c_spec_zeros, c_masked_zeros, "Masked CT should match spec for all zeros");
    ...
    assert_eq!(c_spec_sparse, c_masked_sparse, "Masked CT should match spec for sparse pattern");
    assert!(verify_open(&pp, &c_masked_sparse, &z_sparse), "Sparse pattern should verify");
...
fn precomp_ct_matches_spec_various_patterns() {
    ...
    assert_eq!(c_spec, c_precomp, "Precomp CT commit should match spec for dense pattern");
    assert!(verify_open(&pp, &c_precomp, &z_dense), "Precomp CT commit should verify");
    ...
    assert_eq!(c_spec_mixed, c_precomp_mixed, "Precomp CT should match spec for mixed pattern");
    assert!(verify_open(&pp, &c_precomp_mixed, &z_mixed), "Mixed pattern should verify");
...
fn both_ct_variants_equivalent() {
    ...
    let test_vectors = vec![
        ...
        (0..d*m).map(|i| Fq::from_u64(((i * 13 + 7) % 100) as u64)).collect::<Vec<Fq>>(),
    ...
    for (pattern_num, z) in test_vectors.iter().enumerate() {
        ...
        assert_eq!(c_masked, c_precomp, 
            "Pattern {}: Masked CT and Precomp CT should produce identical results", pattern_num + 1);
        assert_eq!(c_masked, c_spec,
            "Pattern {}: Masked CT should match specification", pattern_num + 1);
        assert_eq!(c_precomp, c_spec,
            "Pattern {}: Precomp CT should match specification", pattern_num + 1);
            
        // Verify all commitments open correctly
        assert!(verify_open(&pp, &c_masked, z), "Pattern {}: Masked CT should verify", pattern_num + 1);
        assert!(verify_open(&pp, &c_precomp, z), "Pattern {}: Precomp CT should verify", pattern_num + 1);
    }
...
fn ct_variants_deterministic() {
    ...
    // Create test input
    let z: Vec<Fq> = (0..d*m).map(|i| match i % 6 {
        0 => Fq::ZERO,
        1 => Fq::ONE,
        2 => Fq::ZERO - Fq::ONE,
        3 => Fq::from_u64(2),
...
fn ct_variants_binding_property() {
    ...
    assert_ne!(c1_masked, c2_masked, "Masked CT: different inputs should produce different commitments");

    // Test precomp CT binding
    let c1_precomp = commit_precomp_ct(&pp, &z1);
    let c2_precomp = commit_precomp_ct(&pp, &z2);
    assert_ne!(c1_precomp, c2_precomp, "Precomp CT: different inputs should produce different commitments");
    ...
    assert!(verify_open(&pp, &c1_precomp, &z1), "Precomp CT c1 should verify with z1");
    assert!(verify_open(&pp, &c2_precomp, &z2), "Precomp CT c2 should verify with z2");

    // Cross-verification should fail
    assert!(!verify_open(&pp, &c1_masked, &z2), "Masked CT c1 should not verify with z2");
    assert!(!verify_open(&pp, &c2_masked, &z1), "Masked CT c2 should not verify with z1");
    assert!(!verify_open(&pp, &c1_precomp, &z2), "Precomp CT c1 should not verify with z2");
    assert!(!verify_open(&pp, &c2_precomp, &z1), "Precomp CT c2 should not verify with z1");
