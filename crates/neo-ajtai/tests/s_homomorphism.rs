...
use neo_ajtai::*;
use neo_math::ring::{Rq as RqEl, cf_inv};
...
fn sample_Z(d: usize, m: usize, b: u32) -> Vec<Fq> {
    ...
    (0..d*m).map(|_| if rng.next_u64() & 1 == 1 { Fq::ONE } else { Fq::ZERO }).collect()
}
...
fn s_module_homomorphism() {
    ...
    let small = |val: i32| -> RqEl {
        let coeffs: [Fq; neo_math::ring::D] = (0..neo_math::ring::D).map(|i| 
            if i==0 { Fq::from_u64(val.rem_euclid(3) as u64) } else { Fq::ZERO }
    ...
    let left = {
        ...
        let mut sum = t1.clone(); sum.add_inplace(&t2); sum
    };
