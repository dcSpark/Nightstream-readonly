...
fn test_real_nova_folding_5_steps() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    for step_i in 0..5 {
        ...
        let is_valid = verify_ivc_step(&step_ccs, &step_result.proof, &current_accumulator, &binding_spec)
...
fn test_folding_equation_validation() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    use neo::ivc::verify_ivc_step;
    let is_valid = verify_ivc_step(&step_ccs, &step_result.proof, &initial_accumulator, &binding_spec)
        .expect("IVC verification should not error");
