...
- `Accepted (SuperNeo path)` means "good enough for the actual SuperNeo theorem route, not yet maximally generalized."
  Example: `S6.3` is accepted because the concrete Goldilocks `paperCarrier`-difference invertibility route needed by SuperNeo is proved, even though the standalone generic invertibility theorem is still open.
...
- `Done (Boundary)` means "this module works if you hand it the right upstream theorem bundle, but that upstream bundle is not fully eliminated yet."
  Example: `S7.3` / `Π_RLC` is proved from `ProtocolTargetAssumptions`, so the reduction step is usable, but the broader protocol-target boundary is still not fully discharged repo-wide.
...
- Remaining open rows are broader library work outside that critical path: `CoeffMaps`, `Decomp`, the generic MLE folding bridge, and the generic CCS definition layer.
- Some rows still marked `Accepted (SuperNeo path)` or `Done (Boundary)` are sufficient for the active paper route but not yet maximally generalized for arbitrary replacement components.
...
| `S6.1` | Defs 5, 9-10, Thm 6 | `InteractiveReductions.lean` | Weak/strong reductions compose correctly. | - | S7.6 | Done (Boundary): strong/weak composition theorems are proved from `InteractiveReductionAssumptions`; on the active Goldilocks final-theorem route the accepted SumCheck witness now feeds the derived witness-level failure package directly, so no extra local SumCheck boundary package remains on the final protocol path. |
| `S6.2` | Defs 4, 16, 18, Thm 2 | `ProofSystem/Lattice.lean`, `LatticeReductions.lean` | Ajtai commitment properties, MSIS hardness, binding reductions. | - | S6.3, S7.6 | Done (Boundary): Defs 4/16/18 and the Thm 2 reduction chain are proved; on the active `paperCarrier`/Goldilocks final-theorem path the Ajtai reduction package is reconstructed theorem-natively from the theorem-level MSIS hardness assumption with proved strong-sampling, while the generic `LatticeReductionLaws` surface remains only for abstract carrier generalization. |
...
| `S6.1` | Done (Boundary). | Optional extension only: instantiate `InteractiveReductionAssumptions` on additional protocol routes beyond the active native Goldilocks path. | Theorem-6 composition remains proved from the reduction bundle and is consumed directly by S7.6. |
| `S6.2` | Done (Boundary). | Optional extension only: discharge the remaining generic `LatticeReductionLaws` abstraction if a fully carrier-parametric reduction library is desired beyond the active `paperCarrier`/Goldilocks route. | MSIS-to-Ajtai binding theorems are consumed by S7.6 after reconstructing the internal boundary package from the theorem-level MSIS hardness assumption on the concrete protocol path. |
...
| `S7.5` | Done (Boundary). | Discharge the remaining upstream theorem providers feeding the arithmetic/protocol-target bundle (currently the accepted `S6.3` carrier/invertibility base). | Arithmetic obligations and protocol-target composition remain theorem-native once those upstream providers are instantiated. |
...
| M26 | Theorem 2 (Ajtai properties) | `ProofSystem/LatticeReductions.lean` | S6.2 | Done (Boundary) |
...
| Done (Boundary) | 11 (M1, M12, M16, M20, M21, M22, M26, M34, M35, M36, M37) |
| Done (Proof-Complete) | 18 (M3, M4, M6, M9, M10, M11, M13, M14, M15, M17, M18, M19, M23, M24, M25, M28, M29, M38) |
| In progress | 7 (M2, M5, M7, M30, M31, M32, M33) |
| Checks green | 1 (M39) |
| Good shell | 0 |
| Not started | 0 |
