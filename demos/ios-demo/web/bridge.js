function postToSwift(msg) {
  try {
    ...
    // Ignore missing handler / posting failures (e.g. when opened in a normal browser).
  }
}
...
function failInflightRuns(error) {
  ...
  for (const id of ids) {
    clearRunWatchdog(id);
    postToSwift({ type: "error", id, error: String(error) });
  }
}
...
function ensureProverWorker() {
  ...
  proverWorker.addEventListener("error", (e) => {
    ...
    postToSwift({ type: "error", id: -1, error: message });
  });
  ...
  proverWorker.addEventListener("message", (ev) => {
    ...
    if (msg?.type === "log") {
      postToSwift({ type: "log", id, line: String(msg?.line ?? "") });
      return;
    }

    if (msg?.type === "error") {
      clearRunWatchdog(id);
      postToSwift({ type: "error", id, error: String(msg?.error ?? "Unknown error") });
      return;
    }
    ...
    if (msg?.type === "done") {
      ...
      postToSwift({ type: "done", id, raw: msg?.raw ?? null, spartan });
    }
  });
...
// Let Swift know the bridge is ready (and include capabilities for UI badges).
(async function () {
  try {
    ensureProverWorker();
    const cap = await getCapabilities();
    postToSwift({ type: "ready", capabilities: cap });
  } catch (e) {
    postToSwift({ type: "error", id: -1, error: String(e) });
  }
})();
